...
impl UserRelationship {
    pub async fn create(user1_id: i32, user2_id: i32, rel_type: UserRelationshipType, pool: &PgPool) -> Result<Self> {
        Ok(
    ...
    pub async fn get(user_id: i32, pool: &PgPool) -> Result<Vec<Self>> {
        Ok(
            sqlx::query_as::<_, Self>("SELECT * FROM user_relationships WHERE user1_id = $1 OR user2_id = $1")
                .bind(user_id)
                .fetch_all(pool)
                .await?
        )
